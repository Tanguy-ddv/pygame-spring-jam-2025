...
class BackgroundSystem:
    def __init__(self):
        self.stars = []
        for i in range(100):
            self.stars.append({"x":random.randint(0, 1280), "y":random.randint(0, 720), "alpha": 0, "time": random.randint(0, 1000), "star_type":str(random.randint(0, 49))})
    ...
    def reset_stars(self, camera):
        self.stars = []
        for i in range(100):
            self.stars.append({"x":random.randint(0, camera.internal_surface.get_size()[0]), "y":random.randint(0, camera.internal_surface.get_size()[1]), "alpha": 0, "time": random.randint(0, 1000), "star_type":str(random.randint(0, 49))})
    ...
    def spawn_new_meteor(self, camera:camera_system.CameraSystem):
        ...
        if screen_border == 0:
            ...
            self.shooting_stars.append({"x":random.randint(int(camera.camera_x - camera.internal_surface.get_size()[0] / 2), int(camera.camera_x + camera.internal_surface.get_size()[0] / 2)), "y":camera.camera_y - camera.internal_surface.get_size()[1] / 2, "direction":direction, "speed":speed, "star_type":str(random.randint(0, 24))})
            ...
            self.shooting_stars.append({"x":camera.camera_x - camera.internal_surface.get_size()[0] / 2, "y":random.randint(int(camera.camera_y - camera.internal_surface.get_size()[1] / 2), int(camera.camera_y + camera.internal_surface.get_size()[1] / 2)), "direction":direction, "speed":speed, "star_type":str(random.randint(0, 24))})
        
        elif screen_border == 3:
            direction = random.randint(135, 225)
            self.shooting_stars.append({"x":int(camera.camera_x + camera.internal_surface.get_size()[0] / 2), "y":random.randint(int(camera.camera_y - camera.internal_surface.get_size()[1] / 2), int(camera.camera_y + camera.internal_surface.get_size()[1] / 2)), "direction":direction, "speed":speed, "star_type":str(random.randint(0, 24))})
    ...
    def update_stars(self, camera:camera_system.CameraSystem, camera_bounding_box:tuple, delta_time:float):
        for star in self.stars:
            if star["x"] < camera_bounding_box[0]:
                ...
                star["y"] = random.randint(int(camera.camera_y - camera.internal_surface.get_size()[1] / 2), int(camera.camera_y + camera.internal_surface.get_size()[1] / 2))
            ...
            if star["y"] < camera_bounding_box[1]:
                ...
                star["x"] = random.randint(int(camera.camera_x - camera.internal_surface.get_size()[0] / 2), int(camera.camera_x + camera.internal_surface.get_size()[0] / 2))
    ...
    def draw(self, surface:pygame.surface.Surface, camera:camera_system.CameraSystem, Images:images.ImageManager):
        self.draw_stars(surface, camera, Images)
        self.draw_meteors(surface, camera, Images)

    def draw_stars(self, surface:pygame.surface.Surface, camera:camera_system.CameraSystem, Images:images.ImageManager):
        for star in self.stars:
    ...
    def draw_meteors(self, surface:pygame.surface.Surface, camera:camera_system.CameraSystem, Images:images.ImageManager):
        for shooting_star in self.shooting_stars:
